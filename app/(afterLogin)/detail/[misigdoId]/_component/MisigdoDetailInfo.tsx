...
const MisigdoDetailInfo = () => {
  ...
  return (
    <>
      ...
      <h2>
        {misigdoItem.data.round[0].restaurant_name} <span>{misigdoItem.data.round[0].cost.toLocaleString()}</span>
      </h2>
      <div className={style.middle_description_area}>
        <div className={style.middle_left_section}>
          <p>메뉴 : {misigdoItem.data.round[0].main_menu_name}</p>
          <p>서브 메뉴 : {misigdoItem.data.round[0].side_menu_name}</p>
          <p>주류 메뉴 : {misigdoItem.data.round[0].drink_name}</p>
          <p>디저트 : {misigdoItem.data.round[0].dessert_name}</p>
        </div>
        <div className={style.middle_right_section}>
          <p>주소 : {misigdoItem.data.round[0].restaurant_addr}</p>
